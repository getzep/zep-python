...
import pytest
from .generated_idempotency_fixture import GeneratedAsyncIdempotencyFixture, GeneratedSyncIdempotencyFixture

from zep_cloud.core.http_client import (
    AsyncHttpClient,
    HttpClient,
    _build_url,
    get_request_body,
...
def _sync_idempotency_fixture(
    status_codes: Optional[List[int]] = None,
) -> Tuple[GeneratedSyncIdempotencyFixture, _DummySyncClient]:
    dummy_client = _DummySyncClient(status_codes)
    ...
    return GeneratedSyncIdempotencyFixture(http_client), dummy_client


def _async_idempotency_fixture(
    status_codes: Optional[List[int]] = None,
) -> Tuple[GeneratedAsyncIdempotencyFixture, _DummyAsyncClient]:
    dummy_client = _DummyAsyncClient(status_codes)
    ...
    return GeneratedAsyncIdempotencyFixture(http_client), dummy_client
...
def test_generated_mutation_gets_uuid4_idempotency_key() -> None:
    client, transport = _sync_idempotency_fixture()

    client.mutation()

    _assert_uuid4(transport.last_request_kwargs["headers"]["Idempotency-Key"])


def test_generated_mutation_reuses_idempotency_key_across_retry(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("zep_cloud.core.http_client.time.sleep", lambda _: None)
    client, transport = _sync_idempotency_fixture([500, 200])

    client.mutation(request_options={"max_retries": 1})

    keys = [request["headers"]["Idempotency-Key"] for request in transport.request_kwargs]
    assert len(keys) == 2
    _assert_uuid4(keys[0])
    assert keys[1] == keys[0]


def test_generated_mutation_preserves_caller_idempotency_key() -> None:
    client, transport = _sync_idempotency_fixture()

    client.mutation(idempotency_key="caller-key")

    assert transport.last_request_kwargs["headers"]["Idempotency-Key"] == "caller-key"


def test_generated_get_read_omits_idempotency_key() -> None:
    client, transport = _sync_idempotency_fixture()

    client.get_read()

    assert "Idempotency-Key" not in transport.last_request_kwargs["headers"]


def test_generated_post_read_omits_idempotency_key() -> None:
    client, transport = _sync_idempotency_fixture()
...
@pytest.mark.asyncio
async def test_async_generated_client_matches_selective_idempotency_contract(monkeypatch: pytest.MonkeyPatch) -> None:
    async def no_sleep(_: float) -> None:
        return None

    monkeypatch.setattr("zep_cloud.core.http_client.asyncio.sleep", no_sleep)
    mutation_client, mutation_transport = _async_idempotency_fixture([500, 200])
    await mutation_client.mutation(request_options={"max_retries": 1})
    mutation_keys = [request["headers"]["Idempotency-Key"] for request in mutation_transport.request_kwargs]
    assert len(mutation_keys) == 2
    _assert_uuid4(mutation_keys[0])
    assert mutation_keys[1] == mutation_keys[0]

    caller_client, caller_transport = _async_idempotency_fixture()
    await caller_client.mutation(idempotency_key="caller-key")
    assert caller_transport.last_request_kwargs["headers"]["Idempotency-Key"] == "caller-key"

    for read in ("get_read", "post_read"):
        read_client, read_transport = _async_idempotency_fixture()
        await getattr(read_client, read)()
        assert "Idempotency-Key" not in read_transport.last_request_kwargs["headers"]

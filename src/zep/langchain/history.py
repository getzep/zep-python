...
class ZepChatMessageHistory(BaseChatMessageHistory):
    ...
    def messages(self) -> List[BaseMessage]:  # type: ignore
        ...
        zep_memory: Optional[ModelsMemory] = self._get_memory()
        if not zep_memory:
            return []
    ...
    @property
    def zep_messages(self) -> Union[List[ModelsMessage], None]:
    ...
    def _get_memory(self) -> Optional[ModelsMemory]:
        """Retrieve memory from Zep"""
        try:
            zep_memory: ModelsMemory = self._client.memory.get(
                session_id=self.session_id, memory_type=self.memory_type
            )

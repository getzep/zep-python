...
async def main() -> None:
    ...
    await client.session.create(session_id=session_id, user_id=user_id, metadata={"foo": "bar"})

    # Update session metadata
    print(f"\n---Updating session: {session_id}")
    await client.session.update(session_id=session_id, metadata={"bar": "foo"})
    ...
    classification = await client.session.classify(
    ...
    summary_result = await client.search.get(session_id, text=query, search_scope="summary")
    print("summaryResult: ", summary_result)

    print("\n---Searching over summaries with MMR Reranking")
    summary_mmr_result = await client.search.get(session_id, text=query, search_scope="summary", search_type="mmr")
    print("summary_mmr_result: ", summary_mmr_result)

    print("\n---Searching over messages using a metadata filter")

    messages_result = await client.search.get(
        session_id,
        text=query,
